...
2.  Suppose you use a binary search tree to store the words from the 
    ...
    > It would take Log2(n) comparisons where n represents the number of words in the dictionary.
...
3.  Suppose you use a prefix tree to store the words from the `dictionary.txt` 
    ...
    > It would take O(n) comparisons in the worst case scenario.

4.  Why is the prefix tree better than a binary search tree for implementing 
    Boggle?

    > With a BST you need to start at the middle of the tree and go either left or right just to find the beginning of the word. This is repeated as you traverse further down. Unlike BSTs, a prefix/trie tree is structured in a way that you index characters numerically and immediately start traversing your way down the tree.

5.  Suppose you do not use `IsPrefix` in your `SolveBoard` implementation.  
    How would that affect the program?

    > If I did not use `IsPrefix`I would significantly increase the calls to `IsWord`and unnecessarily travel a lot more around the board than I would have if I stopped as soon as I know when a prefix to a word isn't valid.
