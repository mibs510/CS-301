...
1. ***What is the advantage of the ChunkList approach as opposed to a standard 
  the link list implementation?*** (1 point)
   > The density of nodes is significantly reduced. 
...
2. ***What would be the implications of increasing the size of ARRAY_SIZE to a 
   ...
   > The implication of increasing ARRAY_SIZE could be seen when traversing through each node when calling functions
   > such as Contains() or Remove(). This means that the algorithm will spend more time at every node
   > but one to look for a specific value. This will drastically increase the time for completion for each call to 
   > Contains() or Remove().
...
4. ***Compare placing a new element into the *FIRST* available empty space 
   ...
   > The disadvantage will be that the algorithm will spend more time searching for the first available empty space 
   > rather than going directly to the tail node. The disadvantage is that it will increase the number of nodes and
